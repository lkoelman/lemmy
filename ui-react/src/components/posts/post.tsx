...
import { Sidebar } from '../listings-page/community-sidebar';
...
export class Post extends Component<any, PostState> {
  ...
  constructor(props: any, context: any) {
    ...
    if (this.props.match.params.comment_id) {
      state.scrolled_comment_id = this.props.match.params.comment_id;
    }
    this.state = state;
  ...
  newComments() {
    return (
      <div className="d-none d-md-block new-comments mb-3 card border-secondary">
  ...
  handleCommentSortChange = (event: any) => {
    this.setState({ commentSort: Number(event.target.value)});
  }
  ...
  parseMessage(msg: WebSocketJsonResponse) {
    ...
    if (msg.error) {
      ...
      this.setState({ moderators : data.moderators });
      ...
      this.setState({ crossPosts: data.posts.filter(
